...
#include <cstdlib>
                    ...
                    Rcpp::NumericVector pxList, double mzLow, double mzHigh, int nThreads)
 {
  ...
  //   m_NPixels=12000;
     ...
     String tmpStr=cv[0];
     const char* peakMethod=tmpStr.get_cstring(); //conversion to C
...
//   std::this_thread::sleep_for(std::chrono::milliseconds(500));   
 ...
 //mtGetGaussians()
 //Parallel processing.
 //Peak are delimited and their Gaussians are formed if peakMode is u/igaussians.
 ...
 { 
   ...
   while(m_enable)
   {
     ...
     if(nPeak>0) //if there are peak to treat
     {
       int px;
       try{
         px=m_spectro[spIndex].pixel; //pixel
...
// printf("..2> %d\n", nPeak);
     ...
     if(m_peakMethod==PeakMethod::uGaussians || m_peakMethod==PeakMethod::iGaussians) //united gaussians mode
     {
       ...
       for(int g=0; g<nGauss; g++)
       {
         ...
         gaussians_p[gaussIndex].mean  =gaussOut.mean;
         gaussians_p[gaussIndex].sigma =gaussOut.sigma;
         gaussians_p[gaussIndex].weight=gaussOut.yFactor*gaussOut.weight;
         gaussIndex++;
       }//end of gaussians
     } 
      else if(m_peakMethod==PeakMethod::derivative) //derivative
      {
        ...
        {
          gaussIn.mean  =m_peakFG_p[px].peakF_p[mPeakLow+pk].max-lowMzIndex ;
          gaussIn.sigma =(m_peakFG_p[px].peakF_p[mPeakLow+pk].high-m_peakFG_p[px].peakF_p[mPeakLow+pk].low+1)/8.0;
          gaussIn.weight=spectro_p->int_p[m_peakFG_p[px].peakF_p[mPeakLow+pk].max];
          gaussIn.yFactor=1.0;
          ...
          gaussians_p[gaussIndex].mean  =gaussOut.mean;
          gaussians_p[gaussIndex].sigma =gaussOut.sigma;
          gaussians_p[gaussIndex].weight=gaussOut.weight;
          gaussIndex++;
        }
      }
...
//   printf("..2> %d\n", gaussIndex);
 ...
 List RawToGaussians::getMeanGaussianSpectrum(double tolerance, int overSampling)
 {
   ...
   double deltaMass=(double)m_mzLow*tolerance/(overSampling*1e6); //delta=1/overSampling of the minimum mass increment of the spectrometer
   int meanAxisSize=1+(m_mzHigh-m_mzLow)/deltaMass;
   
   try{
     if(meanMassAxis_p) {delete [] meanMassAxis_p; meanMassAxis_p=0;}
     meanMassAxis_p= new double[meanAxisSize];
     }
   catch(const std::bad_alloc& e)
   {
     printf("Error reserving memory: %s\n",e.what());
   ...
   bool hit;
   int pxCount=0;
   for(int i=0; i<meanAxisSize; i++) meanMassAxis_p[i]=0.0;
   
   double deltaMass2, gMean, gSigma, gIntensity, mz, den;
   double middle=deltaMass/2.0;
   int vez=1;
   printf("\traw data averaging process (oversampling=%d): ", overSampling);
   
   //for all elements of the spectrum
   for(int px=0; px<m_NPixels; px++)
   {
     //indication of the progress of the process (10% tolerance)
     if((double)px/(double)m_NPixels>vez*0.1) {if(vez<10) printf("%d ", vez*10); vez++;}
     ...
     for(int i=0; i<m_gaussians_p[px].size; i++) 
     {
       ...
       for(int j=iLow; j<iHigh; j++) 
       {
         ...
         double a=gIntensity*exp(-(mz-gMean)*(mz-gMean)/den);
         meanMassAxis_p[j]+=a;
       }
     }
     pxCount++;
   }
   NumericVector meanMz(meanAxisSize), meanInt(meanAxisSize);
   for(int i=0; i<meanAxisSize; i++)
   {
 ...
 //Returns a list with two arrays: averageMz and averageIntensity.
 List RawToGaussians::getMeanSpectrum(double tolerance, int overSampling)
 {
   double highMass, lowMass;
   double *meanMassAxis_p=0;
   ...
   double deltaMass=m_mzLow*tolerance/(overSampling*1e6); 
   int meanAxisSize=1+(m_mzHigh-m_mzLow)/deltaMass;
   ...
   double goodSegment=tolerance*8/1e6;
   int vez=1;
   printf("\traw data averaging process (oversampling=%d): ", overSampling);
   ...
   for(int px=0; px<m_NPixels; px++)
   {
     ...
     {
       double A=m_spectro[0].tmpMass_p[i-1];
       double B=m_spectro[0].tmpMass_p[i];
       
       if(fabs(A-B)>goodSegment*A || A<m_mzLow || B>m_mzHigh) continue;
       idx0 =(A-m_mzLow)/deltaMass; //low  mass point index
       idx1 =(B-m_mzLow)/deltaMass; //high mass point index
       if(idx0<0 || idx1>=meanAxisSize) { break;}
